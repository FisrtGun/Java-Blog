        ...
        sSelect: function () {
            return this.each(function (i, obj) {
                ...
                if (obj.style.display != 'none' && $(this).parents()[0].id.indexOf('__jQSelect') < 0) {
                    ...
                    $('#' + selectId + ' ol').css({width: selectUlwidth + 'px'});
                    ...
                    $('#' + selectId).bind("focus", function () {
                        $.fn.clearSelectMenu(selectId, selectZindex);
                        $('#' + selectId + ' h4').addClass("over");
                    }).bind("click", function (e) {
                        if ($('#' + selectId + ' ol').css("display") == 'block') {
                            $.fn.clearSelectMenu(selectId, selectZindex);
                            return false;
                        } else {
                            ...
                            var ulspace = $('#' + selectId + ' ol').outerHeight(true);
                            var windowspace2 = $(this).offset().top - $(window).scrollTop() - ulspace;
                            windowspace < ulspace && windowspace2 > 0 ? $('#' + selectId + ' ol').css({top: -ulspace}) : $('#' + selectId + ' ol').css({top: $('#' + selectId + ' h4').outerHeight(true)});
                            $(window).scroll(function () {
                                windowspace = ($(window).scrollTop() + document.documentElement.clientHeight) - $('#' + selectId).offset().top;
                                windowspace < ulspace ? $('#' + selectId + ' ol').css({top: -ulspace}) : $('#' + selectId + ' ol').css({top: $('#' + selectId + ' h4').outerHeight(true)});
                            });
...
$.fn.chkCss = function () {
    ...
    $(this).addClass("checkbox_xs");
    $(this).next().addClass("input_check");
    this.each(function () {
        var _this = this;
        var _label = $(_this).next();
        if (_this.checked == true) _label.addClass("input_checks");
        _label.bind("click", function () {
            if (_this.checked == true) {
                if (browser) _this.checked = false;
                _label.removeClass("input_checks");
            }
            else {
                if (browser) _this.checked = true;
                _label.addClass("input_checks");
            }
        });
    });
};
$.fn.rbtCss = function () {
    ...
    var _labelG = $(this).next()
    _labelG.addClass("input_radio");
    ;this.each(function () {
        var _this = this;
        var _label = $(_this).next();
        if (_this.checked == true) _label.addClass("input_radios");
        _label.bind("click", function () {
            _labelG.removeClass("input_radios");
            _label.addClass("input_radios");
            _this.checked = true;
        });
    });
};
$(function () {
    var xOffset = 15;
    var yOffset = -15;
    $("[reg],[url]:not([reg])").hover(function (e) {
        if ($(this).attr('tip') != undefined) {
            var top = (e.pageY + yOffset);
            var left = (e.pageX + xOffset);
            $('body').append('<p id="vtip"><b>' + $(this).attr('tip') + '</b></p>');
...
function change_error_style(obj, action_type) {
    if (action_type == "add") {
        obj.addClass("yz_no").removeClass("yz_yes");
    } else {
        obj.addClass("yz_yes").removeClass("yz_no");
    }
}
